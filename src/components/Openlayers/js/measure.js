...
import deletePoint from '@/assets/MapIcon/deletePoint.png'
import deleteLineArea from '@/assets/MapIcon/deleteLineArea.png'
...
function createDom (point, inner, i, coordinates, lengthMeasureCount) {
  ...
  `<img class="deletePoint" id="deletePoint-${lengthMeasureCount}-${i}" tempimgdata="${lengthMeasureCount}" src="${deletePoint}" style="width: 10px;padding-left: 5px;" />` +
  (i === coordinates.length - 1 ? `<img class="deleteLine" id="deleteLine${lengthMeasureCount}" tempimgdata="${lengthMeasureCount}" src="${deleteLineArea}" style="width: 15px;padding-left: 5px;margin-top: 2px;" />` : '')
...
function deleteLayers (eIndex, target) {
  ...
  for (let i = 0; i < overlaysArr.length; i++) {
    ...
    if (lastChild && lastChild.id && lastChild.getAttribute('tempimgdata') === eIndex) {
...
export default {
        ...
        function (evt) {
          ...
          listener = sketch.getGeometry().on('change', function (evt) {
            ...
            if (geom instanceof Polygon) {
              ...
              measureTooltipElement.innerHTML = output + `<img class="deleteLine" id="deleteLine${lengthMeasureCount}" tempimgdata="${lengthMeasureCount}" src="${deleteLineArea}" style="width: 20px;padding-left: 5px;" />`
            } else if (geom instanceof LineString) {
  ...
  clearToolTip (mapView) {
    const overlaysArr = mapView.getOverlays().getArray()
    const deleteOverlayArr = []
    for (let i = 0; i < overlaysArr.length; i++) {
      const elementText = overlaysArr[i].values_.element
      if (elementText.innerText.includes('公里')) {
        deleteOverlayArr.push(overlaysArr[i])
      }
    }
    deleteOverlayArr.forEach(n => mapView.removeOverlay(n))
    lengthMeasureCount -= 1
  },
  // 删除天气图层
  deleteWeather (mapView) {
    const deleteOverlayArr = []
    const overlaysArr = mapView.getOverlays().getArray()
    for (let i = 0; i < overlaysArr.length; i++) {
      const className = overlaysArr[i].values_.element.className
      if (className && className === 'weather-icon') {
        deleteOverlayArr.push(overlaysArr[i])
      }
    }
    deleteOverlayArr.forEach(n => mapView.removeOverlay(n))
